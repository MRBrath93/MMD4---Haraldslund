...
onMounted(() => {
  ...
  if (cachedbibliotekRaw && cachedTimestampRaw) {
    ...
    if (now - cachedTimestamp < CACHE_DURATION_MS) {
      try {
        bibliotekData.value = JSON.parse(cachedbibliotekRaw);
  ...
  fetch('https://popular-gift-b355856076.strapiapp.com/api/bibliotek?pLevel')
    .then(response => {
      if (!response.ok) {
...
<span>
    ...
    <main v-else>
        ...
        :src="getImage(bibliotekData.reklame_kort.Billede)" 
        :alt="bibliotekData.reklame_kort.Billede.alternativeText" 
        :title="bibliotekData.reklame_kort.Titel" 
        :text="bibliotekData.reklame_kort.Tekst_afsnit" 
        :Btn_title="bibliotekData.reklame_kort.Knapper[0].btn_titel" 
