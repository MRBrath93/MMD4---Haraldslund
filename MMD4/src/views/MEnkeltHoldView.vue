...
const CACHE_DURATION_MS = 60 * 60 * 1000; // 1 time i millisekunder
...
onMounted(async () => {
  ...
  if (cachedRaw && cachedTimestamp && now - Number(cachedTimestamp) < CACHE_DURATION_MS) {
    try {
      motionHoldData.value = JSON.parse(cachedRaw);
  ...
  try {
    const response = await fetch("https://popular-gift-b355856076.strapiapp.com/api/holdoversigt-motionscenter?pLevel");
...
<template>
  <div v-if="classesStore.isLoading || !motionHoldData" class="loading-container">
